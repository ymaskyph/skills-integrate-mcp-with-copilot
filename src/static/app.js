document.addEventListener("DOMContentLoaded", () => {
  ...
  complaintForm.addEventListener("submit", async (event) => {
    ...
    const activity = document.getElementById("complaint-activity").value;
    ...
    try {
      const response = await fetch("/complaints", {
        ...
        body: JSON.stringify({ email, activity, complaint }),
      });
      ...
      setTimeout(() => {
        complaintMessageDiv.classList.add("hidden");
      }, 5000);
    } catch (error) {
  ...
  historyForm.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch(
        `/complaints/user/${encodeURIComponent(email)}`
      );
      ...
      if (complaints.length === 0) {
        ...
        historyList.innerHTML = complaints
          .map(
            (c) => `
          <div class="complaint-card">
            <p><strong>Activity:</strong> ${c.activity}</p>
